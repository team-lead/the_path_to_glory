...
class ProductItem extends Component {
  componentDidMount() {
    if (window.location.href === "http://localhost:3000/product-list/mens") {
      ...
      window.location.href === "http://localhost:3000/product-list/womens"
    ) {
      this.props.getWomensGoods();
    } else if (
      window.location.href === "http://localhost:3000/product-list/accessories"
    ) {
  ...
  getMensGoodsItems = () => {
    return this.props.mensGoods.map(goods => {
      return (
        <Link
  ...
  getWomensGoodsItems = () => {
    return this.props.womensGoods.map(goods => {
      return (
        <Link
          className={classes.linkItem}
          onClick={() => this.props.getDataID(goods)}
  ...
  getAcsGoodsItems = () => {
    return this.props.acsGoods.map(goods => {
      return (
        <Link
          className={classes.linkItem}
          onClick={() => this.props.getDataID(goods)}
